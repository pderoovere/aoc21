...
lines = Path('19/input.txt').read_text().strip('\n')
...
def manhattan(t1, t2):
    return np.sum(np.abs(t1[:3, 3] - t2[:3, 3]))

max_dist = 0
for s1_idx in range(len(scanners) - 1):
    for s2_idx in range(s1_idx + 1, len(scanners)):
        max_dist = int(max(max_dist, manhattan(flattened_transformations[s1_idx], flattened_transformations[s2_idx])))

print('Answer', max_dist)